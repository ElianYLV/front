...
export default function PokemonGen1() {
  ...
  const API_URL = "http://localhost:3001/api/pokemongen1";
    ...
    <div className="min-h-screen p-6" style={{ backgroundImage: 'url(/fondo4.jpg)', backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }}>
      <div className="max-w-6xl mx-auto p-6 bg-white/80 backdrop-blur rounded-2xl shadow-2xl border border-slate-200">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div>
            <h1 className="text-4xl font-extrabold text-indigo-700 tracking-tight">{editId ? "Editar" : "Pokémon Gen 1"}</h1>
            <p className="text-sm text-slate-600">Administra pokémons de la generación 1 con tabla de datos y acciones.</p>
          </div>
          <button
            onClick={clearForm}
          ...
          <div>
            <div className="overflow-x-auto bg-white rounded-xl border border-slate-200 shadow-sm">
              <table className="min-w-full text-left text-sm text-slate-600">
                ...
                <tbody>
                  {pokemons.length === 0 ? (
                    <tr><td className="px-4 py-4" colSpan={6}>No hay pokémons.</td></tr>
                  ) : (
                    pokemons.map((p) => (
